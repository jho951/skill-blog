(function (window, document) {
    ...
    var HIDE_SELECTOR = ['#bnr', '#ttTopAd', '#ttBackdropAdSlotArticle', '#ttBackdropAdSlotList', '.tt-backdrop-ad-shell', '.tt-backdrop-ad-center', '.backdrop-ad-slot', '.google-auto-placed', 'ins.adsbygoogle', 'iframe[id^="google_ads_iframe"]', 'iframe[src*="doubleclick.net"]', 'iframe[src*="googlesyndication.com"]', '[data-crto-id]', 'a[href*="cat.jp2.as.criteo.com"]', 'a[href*="adclick.g.doubleclick.net"]'].join(', ');
    var SIGNAL_SELECTOR = ['[data-crto-id]', 'a[href*="cat.jp2.as.criteo.com"]', 'a[href*="adclick.g.doubleclick.net"]', 'iframe[id^="google_ads_iframe"]', 'iframe[src*="doubleclick.net"]', 'iframe[src*="googlesyndication.com"]', 'ins.adsbygoogle', 'script[src*="criteo.com"]', 'script[src*="doubleclick.net"]', 'script[src*="googlesyndication.com"]'].join(', ');
    var ROOT_SELECTOR = ['#bnr', '#ttTopAd', '#ttBackdropAdSlotArticle', '#ttBackdropAdSlotList', '.tt-backdrop-ad-shell', '.tt-backdrop-ad-center', '.backdrop-ad-slot', '.google-auto-placed', '.ad-wrapper', 'ins.adsbygoogle'].join(', ');
    ...
    function removeAdNode(node) {
        ...
        if (!isElement(root) || root.dataset.ttAdGuardRemoved === 'true') return;
    ...
    function scan(root) {
        if (!root) return;
        if (isElement(root) && hasAdSignals(root)) { removeAdNode(root); return; }
        ...
        if (typeof ns.qsa === 'function') {
            ...
            Array.prototype.forEach.call(root.querySelectorAll(HIDE_SELECTOR), removeAdNode);
            Array.prototype.forEach.call(root.querySelectorAll(SIGNAL_SELECTOR), removeAdNode);
        }
    }
